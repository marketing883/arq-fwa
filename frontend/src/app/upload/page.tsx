          ...
          <div key={step.number} className="flex items-center gap-2">
            {idx > 0 && (
              <div
                className={cn(
                  "h-px w-10 sm:w-16",
                  isComplete || isActive ? "bg-arq-blue-500" : "bg-surface-3",
                )}
              />
            )}
              ...
              <div
                className={cn(
                  ...
                  isComplete
                    ...
                    : isActive
                      ? "bg-arq-blue-500 text-white ring-4 ring-arq-blue-500/20"
                      : "bg-surface-3 text-t-muted",
                )}
              >
                {isComplete ? (
...
export default function UploadPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Page header */}
      <div>
        <h1 className="text-2xl font-bold text-t-primary">Upload Claims Data</h1>
      ...
      {step === 1 && (
        <div className="glass-card rounded-lg border border-white/[0.06]">
          <div className="border-b border-white/[0.06] px-6 py-4">
            <h2 className="text-lg font-semibold text-t-primary">Create Workspace</h2>
            <p className="mt-1 text-sm text-t-muted">
          ...
          <div className="px-6 py-6 space-y-5">
            {/* Name */}
            <div>
              <label htmlFor="ws-name" className="block text-sm font-medium text-t-secondary mb-1">
              ...
              <input
                ...
                className="block w-full rounded-md bg-surface-2 border border-surface-3 text-t-primary px-3 py-2 text-sm placeholder:text-t-muted focus:border-arq-blue-500 focus:outline-none focus:ring-1 focus:ring-arq-blue-500"
              />
            </div>

            {/* Client name */}
            <div>
              <label htmlFor="ws-client" className="block text-sm font-medium text-t-secondary mb-1">
                Client Name
              </label>
              <input
                id="ws-client"
            ...
            {/* Description */}
            <div>
              <label htmlFor="ws-desc" className="block text-sm font-medium text-t-secondary mb-1">
                Description
              </label>
              <textarea
                id="ws-desc"
                ...
                {(["medical", "pharmacy"] as const).map((ct) => (
                  <button
                    ...
                    className={cn(
                      ...
                      claimType === ct
                        ? "bg-arq-blue-500 text-white"
                        : "bg-surface-2 text-t-secondary border border-surface-3 hover:bg-surface-2/40",
                    )}
                  >
                    {ct}
                  </button>
            ...
            <div>
              ...
              <div
                ...
                className={cn(
                  ...
                  dragActive
                    ? "border-arq-blue-500 bg-arq-blue-500/10"
                    : file
                      ? "border-green-500/30 bg-green-500/10"
                      : "border-surface-3 hover:border-arq-blue-500/50 bg-surface-2",
                )}
              >
                <input
                  ref={fileInputRef}
                ...
                {file ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    <Upload className="h-10 w-10 text-t-muted" />
          ...
          <div className="flex justify-end border-t border-white/[0.06] px-6 py-4">
            <button
              ...
              className={cn(
                ...
                loading
                  ? "cursor-not-allowed bg-arq-blue-500/50"
      ...
      {step === 2 && preview && (
        <div className="glass-card rounded-lg border border-white/[0.06]">
          <div className="border-b border-white/[0.06] px-6 py-4">
            <h2 className="text-lg font-semibold text-t-primary">Column Mapping</h2>
            <p className="mt-1 text-sm text-t-muted">
              We detected <span className="font-medium text-t-secondary">{preview.total_rows}</span>{" "}
              rows and <span className="font-medium text-t-secondary">{preview.columns.length}</span>{" "}
          ...
          <div className="px-6 py-6">
            <div className="overflow-x-auto rounded-lg border border-white/[0.06]">
              <table className="dark-table w-full text-sm">
                <thead>
                  <tr className="bg-surface-2 text-left text-t-secondary border-b border-white/[0.06]">
                ...
                <tbody className="divide-y divide-white/[0.06]">
                  {preview.columns.map((col) => {
                    const samples = preview.preview_rows
                      ...
                      .filter(Boolean);
                    return (
                      <tr key={col} className="hover:bg-surface-2/40">
                        ...
                        <td className="px-4 py-3">
                          <select
                            ...
                            className="block w-full rounded-md bg-surface-2 border border-surface-3 text-t-primary px-2 py-1.5 text-sm focus:border-arq-blue-500 focus:outline-none focus:ring-1 focus:ring-arq-blue-500"
                          >
                        ...
                        <td className="px-4 py-3 text-xs text-t-muted truncate max-w-xs">
                          {samples.join(", ") || "--"}
                        </td>
                      </tr>
                    );
          ...
          {preview.preview_rows.length > 0 && (
            <div className="px-6 pb-6">
              <h3 className="text-sm font-semibold text-t-secondary mb-2">Data Preview</h3>
              <div className="overflow-x-auto rounded-lg border border-white/[0.06]">
                <table className="dark-table w-full text-xs">
                  <thead>
                    <tr className="bg-surface-2 text-left text-t-secondary border-b border-white/[0.06]">
                      {preview.columns.map((col) => (
                    ...
                    {preview.preview_rows.slice(0, 5).map((row, i) => (
                      <tr key={i} className="hover:bg-surface-2/40">
                        {preview.columns.map((col) => (
                          <td
                            key={col}
                            className="px-3 py-2 text-t-secondary whitespace-nowrap max-w-[200px] truncate"
                          >
                            {row[col] ?? ""}
                          </td>
                        ))}
          ...
          <div className="flex justify-between border-t border-white/[0.06] px-6 py-4">
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-2 rounded-md border border-surface-3 bg-surface-2 px-4 py-2.5 text-sm font-medium text-t-secondary transition-colors hover:bg-surface-2/40"
            >
      ...
      {step === 3 && ingestResult && (
        <div className="glass-card rounded-lg border border-white/[0.06]">
          <div className="border-b border-white/[0.06] px-6 py-4">
            <h2 className="text-lg font-semibold text-t-primary">Ingest Complete</h2>
          </div>
            ...
            <div className="mx-auto max-w-md text-center space-y-6">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-t-muted">
                  ...
                  <span className="font-mono font-medium text-t-secondary">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="rounded-lg bg-surface-2 p-4">
                  ...
                  <p className="text-xs text-t-muted mt-1">Rows Imported</p>
                </div>
                <div className="rounded-lg bg-surface-2 p-4">
                  <p className="text-2xl font-bold text-t-primary">
                    {ingestResult.rows_skipped.toLocaleString()}
                  </p>
                  <p className="text-xs text-t-muted mt-1">Rows Skipped</p>
                </div>
                <div className="rounded-lg bg-surface-2 p-4">
                  <p className="text-2xl font-bold text-t-primary capitalize">
                    {ingestResult.claim_type}
                  </p>
                  <p className="text-xs text-t-muted mt-1">Claim Type</p>
                </div>
              </div>
              ...
              <div className="flex justify-center gap-3 pt-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-md bg-arq-blue-500 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-arq-blue-600"
                >
                  View Claims
                  <ArrowRight className="h-4 w-4" />
                </button>
